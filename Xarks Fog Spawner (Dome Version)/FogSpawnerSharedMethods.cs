...
public class FogSpawnerSharedMethods : IHoldfastSharedMethods
{
    ...
    public void PassConfigVariables(string[] value)
    {
        ...
        {
            fogController.fogColor = Color.gray; // Can replace with parsed config values
            fogController.minFog = 0.1f;
            fogController.fogDensity = 0.6f;
            fogController.ApplyFogSettings();
        }
    }
    ...
    public void OnRoundDetails(int roundId, string serverName, string mapName, FactionCountry attacker, FactionCountry defender, GameplayMode mode, GameType type)
    {
        ...
        fogDomeController.SpawnFogDome();
    }

    // Required Holdfast interface methods
